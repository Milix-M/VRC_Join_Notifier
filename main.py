...
def createaboutapp(): #このアプリについてのウィンドウを作成する関数
    ...
    aboutapp.geometry("300x160")
    aboutapp.resizable(False, False)
    ...
    booth = tk.Label(aboutapp, text="Boothページ", fg="blue")
    booth.pack()
    booth.bind("<Button-1>", lambda e: webbrowser.open_new("https://reoshop.booth.pm/items/3757293"))
...
def createsettingwin(): #設定ウィンドウを作成する関数
    ...
    writelogchkbox = tk.Checkbutton(settingwin, variable=bl2, text="Join/Leaveログを.txt形式で書き出す").pack()
    bl3 = tk.BooleanVar()
    bl3.set(config["restorelogs"])
    restorelogschkbox = tk.Checkbutton(settingwin, variable=bl3, text="Join/Leaveログを.txtファイルから読み込み復元する").pack()
    bl4 = tk.BooleanVar()
...
appversion = "1.1.2" #アプリのバージョンを設定する
